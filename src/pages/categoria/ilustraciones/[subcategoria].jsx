...
import subcategories from "@/data/subcategories_illustrations";
...
function Subcatgory({ items = [], subcategory }) {
  return (
    <Layout metaData={{ title: `Subcategoría | ${subcategory}` }}>
      <HeroCategory
        background="/img/category/bg_subcat_moderno.png"
        search={true}
        title={subcategory}
        desc=""
        layer={{
        ...
        <div className="row">
          {items.map((item, index) => (
            <div key={index} className="col-4">
              <SubcategoryCard url={`/search/fotos/${item.slug}`} {...item} />
            </div>
          ))}
        </div>
      </section>
